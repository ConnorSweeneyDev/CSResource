...
int try_main(int argc, char *argv[])
{
  ...
  if (state.task == csr::base::state::COMPILE)
  {
    ...
    std::string include_content = "// This file is automatically generated, do not edit manually.\n\n"
                                  ...
                                  "{\n"
                                  "  template <size_t dxil_size, size_t spirv_size> struct compiled_shader\n"
                                  "  {\n"
                                  "    const std::array<unsigned char, dxil_size> dxil = {};\n"
                                  "    const std::array<unsigned char, spirv_size> spirv = {};\n"
                                  "  };\n"
                                  "  template <size_t image_size> struct compiled_texture\n"
                                  "  {\n"
                                  "    const std::array<unsigned char, image_size> image = {};\n"
    ...
    std::for_each(
      ...
      [&state, &exceptions, &exceptions_mutex, &io_mutex](csr::base::resource &resource)
      {
        ...
        if (resource.type == csr::base::resource::SHADER)
        {
          ...
          resource.include_text += "  extern const compiled_shader<" + std::to_string(compiled_dxil_size) + ", " +
                                   std::to_string(compiled_spirv_size) + "> " + resource.path.stem().string() +
                                   ((resource.path.extension() == ".vert") ? "_vertex" : "_fragment") + ";\n";
          resource.source_text += "  const compiled_shader<" + std::to_string(compiled_dxil_size) + ", " +
                                  std::to_string(compiled_spirv_size) + "> " + resource.path.stem().string() +
                                  ((resource.path.extension() == ".vert") ? "_vertex" : "_fragment") + " = {\n    {" +
                                  dxil_data + spirv_data;
          SDL_free(file_data);
        }
        ...
        if (resource.type == csr::base::resource::TEXTURE)
        {
          ...
          resource.include_text += "  extern const compiled_texture<" + std::to_string(width * height * channels) +
                                   "> " + resource.path.stem().string() + "_texture;\n";
          resource.source_text += "  const compiled_texture<" + std::to_string(width * height * channels) + "> " +
                                  resource.path.stem().string() + "_texture = {\n    {" + image_data_hex + "},\n    " +
                                  std::to_string(width) + ",\n    " + std::to_string(height) + ",\n    " +
                                  std::to_string(channels) + ",\n    " + std::to_string(frame_data[0]) + ",\n    " +
                                  std::to_string(frame_data[1]) + ",\n    " + std::to_string(frame_data[2]) + "};\n";
        }
      });
    if (!exceptions.empty())
    {
