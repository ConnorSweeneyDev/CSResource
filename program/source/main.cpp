...
int try_main(int argc, char *argv[])
{
    ...
    std::for_each(
      ...
      [&state, &exceptions, &exceptions_mutex, &io_mutex](csr::base::resource &resource)
      {
        ...
        if (resource.type == csr::base::resource::SHADER)
        {
          ...
          Uint8 *compiled_shader =
            reinterpret_cast<Uint8 *>(SDL_ShaderCross_CompileSPIRVFromHLSL(&hlsl_info, &compiled_size));
          if (!compiled_shader)
          {
            std::lock_guard<std::mutex> lock(exceptions_mutex);
            exceptions.emplace_back(std::make_exception_ptr(
          ...
          }
          {
            ...
            if (SDL_WriteIO(output, compiled_shader, compiled_size) != compiled_size)
            {
              SDL_free(compiled_shader);
              SDL_CloseIO(output);
          ...
          std::string data =
            std::vformat("  static constexpr std::array<const unsigned char, {}> {}_{}_source = {{\n    ",
                         std::make_format_args(compiled_size, name, type));
          for (size_t index = 0; index < compiled_size; ++index)
          {
            data += csr::utility::unsigned_char_to_hex(compiled_shader[index]);
            if (index < compiled_size - 1)
            {
              data += ",";
              if ((index + 1) % 16 == 0)
                data += "\n    ";
          ...
          resource.source_text += std::vformat(
            "{}\n  const compiled_shader {}_{} = {{\n    std::span<const unsigned char>({}_{}_source),\n    {}}};\n",
            std::make_format_args(data, name, type, name, type, compiled_size));
        }
        ...
        if (resource.type == csr::base::resource::TEXTURE)
        {
          ...
          struct csr::base::state::frame_data frame_data = {};
          ...
          while (png.read(buffer, sizeof(buffer)))
          {
            ...
            if (chunk_type == "cSRe")
            {
              ...
              std::regex frame_data_regex(R"(frame_width:\s*(\d+);frame_height:\s*(\d+);((?:\w+:\d+:\d+;)+))");
              std::smatch match;
              if (std::regex_search(comment, match, frame_data_regex))
              {
                frame_data.width = std::stoul(match[1].str());
                frame_data.height = std::stoul(match[2].str());
                std::string groups_string = match[3].str();
                std::regex group_regex(R"((\w+):(\d+):(\d+);)");
                std::sregex_iterator iter(groups_string.begin(), groups_string.end(), group_regex);
                std::sregex_iterator end;
                for (; iter != end; ++iter)
                  frame_data.groups.push_back(
                    {(*iter)[1].str(), std::stoul((*iter)[2].str()), std::stoul((*iter)[3].str())});
              }
              else
              {
                std::lock_guard<std::mutex> lock(exceptions_mutex);
          ...
          if ((static_cast<unsigned int>(width) % frame_data.width != 0 || frame_data.width <= 0 ||
               frame_data.width > static_cast<unsigned int>(width)) ||
              (static_cast<unsigned int>(height) % frame_data.height != 0 || frame_data.height <= 0 ||
               frame_data.height > static_cast<unsigned int>(height)))
          {
            std::lock_guard<std::mutex> lock(exceptions_mutex);
            exceptions.emplace_back(std::make_exception_ptr(csr::utility::exception(
              "Frame dimensions must be a multiple of texture dimensions {}", resource.path.string())));
            return;
          }

          std::string frame_data_floats = "";
          unsigned int total_frames = (static_cast<unsigned int>(width) / frame_data.width) *
                                      (static_cast<unsigned int>(height) / frame_data.height);
          for (const auto &group : frame_data.groups)
          {
            if (group.end_frame < group.start_frame)
            {
              std::lock_guard<std::mutex> lock(exceptions_mutex);
              exceptions.emplace_back(std::make_exception_ptr(
                csr::utility::exception("Invalid frame group specification for {}.png - {}:{}:{}", name, group.name,
                                        group.start_frame, group.end_frame)));
              return;
            }
            if (group.start_frame <= 0 || group.end_frame <= 0 || group.start_frame > total_frames ||
                group.end_frame > total_frames)
            {
              std::lock_guard<std::mutex> lock(exceptions_mutex);
              exceptions.emplace_back(std::make_exception_ptr(csr::utility::exception(
                "Start and end frames must be greater than 0 and less than or equal to total possible frames {}",
                resource.path.string())));
              return;
            }

            const unsigned int frame_count = group.end_frame - group.start_frame + 1;
            frame_data_floats +=
              std::vformat("  static constexpr std::array<const compiled_texture::frame_data::group::frame, {}> "
                           "{}_texture_{}_frames = {{\n    {{",
                           std::make_format_args(frame_count, name, group.name));
            unsigned int frames_per_row = static_cast<unsigned int>(width) / frame_data.width;
            unsigned int frames_per_column = static_cast<unsigned int>(height) / frame_data.height;
            for (unsigned int frame = group.start_frame - 1; frame < group.end_frame; ++frame)
            {
              unsigned int frame_x = frame % frames_per_row;
              unsigned int frame_y = (frames_per_column - 1) - (frame / frames_per_row);
              const float top = static_cast<float>((frame_y + 1) * frame_data.height) / static_cast<float>(height);
            ...
            frame_data_floats += "}};";
            if (&group != &frame_data.groups.back()) frame_data_floats += "\n";
          }
          ...
          resource.source_text +=
            std::vformat("{}\n{}\n  const compiled_texture {}_texture = {{\n    std::span<const unsigned "
                         "char>({}_texture_image),\n    {{{}, {}, {}}},\n    {{{}, {},\n     {{",
                         std::make_format_args(image_data_hex, frame_data_floats, name, name, width, height, channels,
                                               frame_data.width, frame_data.height));
          for (const auto &group : frame_data.groups)
          {
            resource.source_text +=
              std::vformat("{{\"{}\", {{{}, {}, std::span<const "
                           "compiled_texture::frame_data::group::frame>({}_texture_{}_frames)}}}}",
                           std::make_format_args(group.name, group.start_frame, group.end_frame, name, group.name));
            if (&group != &frame_data.groups.back()) resource.source_text += ",\n      ";
          }
          resource.source_text += "}}};\n";
        }
      });
  ...
  else if (state.task == csr::base::state::LIST)
  {
    ...
    std::unordered_map<std::string, struct csr::base::state::frame_data> frame_data = {};
      ...
      if (entry.is_regular_file() && entry.path().extension() == ".png")
      {
        ...
        while (png.read(buffer, sizeof(buffer)))
        {
          ...
          if (chunk_type == "cSRe")
          {
            ...
            if (std::regex_search(comment, match, frame_data_regex))
            {
              struct csr::base::state::frame_data data_entry = {};
              data_entry.width = std::stoul(match[1].str());
              data_entry.height = std::stoul(match[2].str());
              std::string groups_string = match[3].str();
              std::regex group_regex(R"((\w+):(\d+):(\d+);)");
              std::sregex_iterator iter(groups_string.begin(), groups_string.end(), group_regex);
              std::sregex_iterator end;
              for (; iter != end; ++iter)
                data_entry.groups.push_back(
                  {(*iter)[1].str(), std::stoul((*iter)[2].str()), std::stoul((*iter)[3].str())});
              frame_data[entry.path().string()] = std::move(data_entry);
            }
            else
        ...
        if (!found_chunk) frame_data[entry.path().string()] = {};
        png.close();
      }
    }
    std::string output = {};
    for (const auto &entry : frame_data)
      ...
      {
        output = std::format("{} | {}x{}:\n", entry.first, entry.second.width, entry.second.height);
        for (const auto &group : entry.second.groups)
          output += std::format("  - [{} | first:{} last:{}]\n", group.name, group.start_frame, group.end_frame);
      }
    for (const auto &entry : frame_data)
      if (entry.second.width == 0 && entry.second.height == 0) output = std::format("{}: NULL", entry.first);
  ...
  else if (state.task == csr::base::state::SET)
  {
    ...
    std::string chunk_data =
      std::format("frame_width:{};frame_height:{};", state.frame_data.width, state.frame_data.height);
    for (const auto &group : state.frame_data.groups)
      chunk_data += std::format("{}:{}:{};", group.name, group.start_frame, group.end_frame);
