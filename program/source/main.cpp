...
int try_main(int argc, char *argv[])
{
    ...
    std::for_each(
      ...
      [&state, &exceptions, &exceptions_mutex, &io_mutex](csr::base::resource &resource)
      {
        ...
        if (resource.type == csr::base::resource::TEXTURE)
        {
          ...
          while (png.read(buffer, sizeof(buffer)))
          {
            ...
            if (chunk_type == "cSRe")
            {
              ...
              else
              {
                std::lock_guard<std::mutex> lock(exceptions_mutex);
                exceptions.emplace_back(std::make_exception_ptr(
                  csr::utility::exception("Texture file must have frame_width metadata {}", resource.path.string())));
                return;
              }
              found_chunk = true;
              break;
          ...
          if (width % frame_width != 0 || frame_width <= 0 || frame_width > width)
          {
            std::lock_guard<std::mutex> lock(exceptions_mutex);
            exceptions.emplace_back(std::make_exception_ptr(
              csr::utility::exception("Texture width must be a multiple of frame width {}", resource.path.string())));
            return;
          }
          int frame_count = width / frame_width;
  ...
  else if (state.task == csr::base::state::LIST)
  {
    ...
    std::unordered_map<std::string, int> frame_widths;
      ...
      if (entry.is_regular_file() && entry.path().extension() == ".png")
      {
        ...
        while (png.read(buffer, sizeof(buffer)))
        {
          ...
          if (chunk_type == "cSRe")
          {
            ...
            std::smatch match;
            if (std::regex_search(comment, match, frame_width_regex))
              frame_widths[entry.path().string()] = std::stoi(match[1].str());
            else
        ...
        if (!found_chunk) frame_widths[entry.path().string()] = 0;
        png.close();
      }
    }
    for (const auto &entry : frame_widths)
      if (entry.second > 0) std::cout << entry.first << ": " << entry.second << std::endl;
    for (const auto &entry : frame_widths)
      if (entry.second == 0) std::cout << entry.first << ": NULL" << std::endl;
  }
  else if (state.task == csr::base::state::SET)
  {
    ...
    size_t iend_pos = std::string(file_data.begin(), file_data.end()).find("IEND");
    if (iend_pos == std::string::npos)
      throw csr::utility::exception("Invalid PNG file, IEND chunk not found {}", state.texture_path.string());

    std::string chunk_data = "frame_width: " + std::to_string(state.frame_width);
